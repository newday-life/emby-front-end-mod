...
class HomeSwiper {
    static start() {
        ...
        // this.itemQuery.OfficialRatings = "NC-17|JP-18+|XXX";//只要18禁
        // delete this.itemQuery.SortOrder, this.itemQuery.SortBy = "Random";//全局随机·
        // this.itemQuery.HasTmdbId = true;// true为TheMovieDb削刮到的影视(应可排除jav之类的)
    ...
    static async playlistener() {
        ...
        document.querySelectorAll(".view:not(.hide) .banner-item-play").forEach(async (dom, i) => {
            ...
            // const detail = this.Alldata.filter(m => m.data.filter(m => m.Id === id)[0])[0].data.filter(m => m.Id === id)[0];
    ...
    static async initBanner() {
        ...
        // this.Alldata = this.Alldata.filter(m => m.data.length > 0);
        ...
        this.swiper = new Swiper(".view:not(.hide) .mySwiper", {
            loop: true,
            spaceBetween: 20,
            slidesPerView: 2,/*修改的话on监听，breakpoint函数也要修改判断*/
            freeMode: true,
        ...
        this.swiper2 = new Swiper(".view:not(.hide) .mySwiper-main", {
            ...
            on: {
                ...
                keyPress: function (swiper, event) {
                    if (event == "Enter" || event === 13) {
                        let selectedElement = document.querySelector('.view:not(.hide) .swiper-slide-thumb-active .small-banner');
                        if (selectedElement) {
                            Emby.Page.showItem(selectedElement.id);
                        }
                    }
                },
                /* 横向切换 */
